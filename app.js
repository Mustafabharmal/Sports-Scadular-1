...
app.get('/adminreport', requireAdmin, async (req, res) => {
  if (req.session.bol == null) {
    res.render('report', { bol: false, csrfToken: req.csrfToken() });
  } else {
    const sessionsPlayed = req.session.sessionsPlayed || null;
    const sports = req.session.sports || null;
    const bol = req.session.bol || false;
...
app.post('/adminreport', requireAdmin, async (req, res) => {
  let startdate = moment
    .tz(req.body.startdate, 'YYYY-MM-DD', 'Asia/Kolkata')
    .startOf('day')
    .format('YYYY-MM-DD HH:mm');

  let enddate = moment
    .tz(req.body.enddate, 'YYYY-MM-DD', 'Asia/Kolkata')
    .endOf('day')
    .format('YYYY-MM-DD HH:mm');
  ...
  sessionsPlayed = sessionsPlayed.map(session => ({
    ...session.toJSON(),
    when: formatTimestamp(session.when),
  }));
  ...
  req.session.sessionsPlayed = sessionsPlayed;
  req.session.sports = sports;
  req.session.bol = true;

  res.redirect('/adminreport');
});
...
app.post('/session', (req, res, next) => {
  ...
  request(options, (err, response, body) => {
    ...
    // reCaptcha verification succeeded, proceed with authentication
